...
public class BattleField {
  ...
  /*
   * The following are temporarily disabled functions for migrating income enemy reinforcements.
   * 
   * private void propagate(int remainingEnemies) { int temp; if(remainingEnemies > 0) { for(int i =
   * 0; i < outerSectors.length; i++) { temp = reinforceOuter(remainingEnemies/4, outerSectors[i]);
   * if(temp > 0) { reinforceInner((int)temp/2, outerSectors[i].one()); reinforceInner((int)temp/2,
   * outerSectors[i].two()); } } } }
   * 
   * private int reinforceFront(int enemy) { if(!front.enemyCanPass()) { front.addEnemy(enemy);
   * return 0; } if(front.allyProportion() == 0) { return enemy; } int remainder = 0; for(int i = 0;
   * i < 5; i++) { double move = Math.random(); if(move >= 0.5) { remainder += (int) (enemy / 5); }
   * else { front.addEnemy((int)(enemy/5)); } } return remainder; }
   * 
   * private int reinforceOuter(int enemy, OuterSector outer) { if(!outer.enemyCanPass()) {
   * outer.addEnemy(enemy); return 0; } if(outer.allyProportion() == 0) { return enemy; } int
   * remainder = 0; for(int i = 0; i < 3; i++) { double move = Math.random(); if(move >= 0.5) {
   * remainder += (int) (enemy / 3); } else { outer.addEnemy((int)(enemy/3)); } } return remainder;
   * }
   * 
   * private void reinforceInner(int enemy, InnerSector inner) { if(!inner.enemyCanPass()) {
   * inner.addEnemy(enemy); return; } if(inner.allyProportion() == 0) { tower.addEnemy(enemy); }
   * for(int i = 0; i < 2; i++) { double move = Math.random(); if(move >= 0.5) {
   * tower.addEnemy((int) (enemy / 2)); } else { inner.addEnemy((int)(enemy/2)); } } }
   */
}
